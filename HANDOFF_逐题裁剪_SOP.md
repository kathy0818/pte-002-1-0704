...
- **口语(Speaking)：只裁 DI（Describe Image / 看图说话）**，其它口语题型（RS/RL/ASQ/SGD/RA…）**不裁**。
...
> **范围说明 / Scope note：** 这些图是 **PTE 学术类(Academic)** 的模考。用户实际考的是 **PTE Core**，并且**明确要求口语只保留 DI**（用户自己划定的复习范围，不是「Core 口语只有 DI」的题型断言——PTE Core 与 Academic 题型结构不同）。所以下表「裁不裁」= **本任务的取舍**：听力/阅读全裁，口语按用户要求**只裁 DI**。若用户改口径（比如也要 RA/ASQ），按用户最新说的来。

| 代码 | 全称 | 所属部分 | 本任务裁不裁 |
|---|---|---|---|
| DI | Describe Image 看图说话 | Speaking | ✅ **口语只裁这个** |
| RS | Repeat Sentence | Speaking | ❌ 跳过 |
| RA | Read Aloud | Speaking | ❌ 跳过 |
| RL | Retell Lecture | Speaking | ❌ 跳过 |
| ASQ | Answer Short Question | Speaking | ❌ 跳过 |
| SGD | Situation / Group Discussion | Speaking | ❌ 跳过 |
...
| SST | Summarize Spoken Text | Listening | ✅ 裁 |
...
> 口语页里 DI 一般排在 RS 之后（例：口语页第 17–21 题才是 DI）。**认标题里的 `DI` 代码，不要靠位置猜。**
...
```
original_screenshots/                 # 原始整页长图，原封不动，只搬进来
...
cropped_questions/                    # 逐题裁剪成品（要交付的）
    ...
    speaking_DI/  DI01_… DI02_… …     # 口语：只放 DI
```

**命名规则：** `前缀+序号_题型_题库编号_标题.png`

- 前缀：听力 `L`、阅读 `R`、口语 DI 用 `DI`。
- 序号：**两位补零**，跨图连续（`L01`…`L15`）。
- 题型/编号/标题：来自标题行（`11. HIW #216 T Cell` → `L11_HIW_216_T_Cell`）。
- 标题里空格→`_`；去掉 `#`；`&` 去掉或写成词（`FIBD&D`→`FIBDD`）；文件名只留 `A-Za-z0-9-_`。
- 没有独立标题的（如 `WFD #3208 WFD`）就省略标题：`L13_WFD_3208.png`。
...
def headers(fn):
    ...
    for ws in lines.values():
        ...
        if m and 820<=left<=990:                         # 靠左的题号列
            num=int(m.group(1))
            if 1<=num<=40 and (re.search(TYPES,text) or '#' in text):
                out.append((top,num,text[:60]))
...
### 步骤 C — 水平内容列（X 范围）
APEUni 页面内容居中在一列里。**本套图实测 `X = [835, 1912]`**（`835` 含左侧题号，`1912` 刚好含右上角 `查看原题 >`）。

新图若缩放不同要复核两件事：
- 右边界要**恰好包住** `查看原题 >`（实测其右端 ≈ x1909）；
- 又要**尽量排除**右侧浮动按钮（左缘 ≈ x1900+）。二者略有重叠，取 `1912` 兼顾。
```python
# 快速验证内容列：看某竖直带里非白像素的左右范围
import numpy as np
a=np.asarray(Image.open('听力2.jpeg').convert('RGB'))
band=a[900:3900]; col=(band.min(axis=2)<245).sum(axis=0)
# 打印 col 的分布，找主体连续块的左右缘即可
```
...
def compute_end(a, start):        # a=整张图ndarray, start=最后一题题号Y
    ...
    footer=next((i for i in range(len(dark)) if dark[i]>0.4), len(dark))   # 深色页脚起点
    teal=(G>140)&(R<130)&(B>120)&(G-R>45)                                  # 青色胶囊
    z=teal[:footer, 835:835+180].sum(axis=1)                              # 只看最左侧(评分详情胶囊)
    rows=np.where(z>6)[0]
...
整页截图里会**夹着一条站点导航条**（`课程 社区 APP 机构版 留学 … 考试报名优惠¥2089 … PTE Core` + 大猩猩头像），出现在**截图时的滚动位置**，一张图里通常**只出现一次**。
...
### 步骤 G — SST / 弹窗类评分详情
若单独截了某题的「AI 评分」弹窗（白色对话框浮在灰底上），把**白色对话框**裁出来即可（去掉左右灰底），命名挂到对应题：`L01b_SST_712_AI_scoring_detail.png`。
...
### 6.6 缩放/分辨率不同
若新图不是 2372 宽，`X=[835,1912]`、各种阈值要按比例重算或用 §5-C 的方法重测。**别照抄像素值，照抄方法。**

### 6.7 跨图题号连续
一个部分分多张图时，第 2 张接着第 1 张的题号编号（不要每张都从 1 开始）。

### 6.8 口语只留 DI
再强调：口语页里 RS/RL/ASQ/SGD **全部跳过**，只留 `DI`。
...
NAV   = {'听力1.jpeg':[(1103,1182)], '听力2.jpeg':[(3243,3322)], ...}
FLOAT = {'阅读1.jpeg':[(2843,2948,1866)], ...}   # (y0,y1,x_left)
...
def compute_end(a,start):              # 见 §5-D
    H=a.shape[0]; sub=a[start:H].astype(int); R,G,B=sub[:,:,0],sub[:,:,1],sub[:,:,2]
    dark=(sub.max(axis=2)<90).mean(axis=1)
    footer=next((i for i in range(len(dark)) if dark[i]>0.4), len(dark))
...
1. **数量**：听力/阅读题数对不对？口语是否只剩 DI？跨图题号是否连续无缺号？
2. **Contact sheet**：把所有成品缩略拼成一张大图，一次性核对每张的**上下边界**（有没有切到标题、有没有混进下一题、有没有页脚/翻页条）。
```python
import glob, os
...
4. **已知缺口**：所有「被导航条挡住」的题都加了提示条并写进 README 了吗？
...
- 原图 `git mv` 进 `original_screenshots/`（保留改名记录）。
...
- [ ] 原图全部搬进 `original_screenshots/`，未改动。
- [ ] `cropped_questions/{listening,reading,speaking_DI}/` 齐全，命名规范、题号连续。
- [ ] 听力+阅读**每题**一张；口语**只**有 DI。
- [ ] 每张都从题号裁到评分行；无页脚、无翻页条、无导航条、无浮动按钮残留。
- [ ] 被导航条挡住的内容都加了**黄色提示条** + README「已知缺口」记录，**没有脑补编造**。
- [ ] 跑过 contact sheet 并逐张目检通过。
- [ ] 一份**中英双语 README** 说明结构 + 题目清单 + 已知缺口。
...
本套已完成的成品和全部脚本见分支：
`claude/pte-core-crop-20260704-033730`（`cropped_questions/` 是成品，README 有完整题目清单和「已知缺口」示例，R01/R14 是提示条的实际范例）。
